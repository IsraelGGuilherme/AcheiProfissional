...
<?= $this->section('title') ?>Verificação de E-mail - Achei Profissional<?= $this->endSection() ?>
            ...
            <div class="register-card">
                ...
                <div class="text-center mb-4">
                    ...
                    <p class="welcome-text">
                        Para ativar a sua conta com segurança, precisamos confirmar seu endereço de e-mail.
                    </p>
                </div>

                <!-- Box exibindo o e-mail cadastrado -->
                ...
                <div id="sectionEnviarCodigo" class="mb-3">
                    <button
                        ...
                        <span id="btnEnviarTexto">Enviar código para meu e-mail</span>
                    </button>
                </div>

                <!-- Sessão 2: Informar código recebido (Exibida após o primeiro envio com sucesso) -->
                <div id="sectionInformarCodigo" class="d-none fade-in">
                    ...
                    <form action="<?= base_url('verificaremail') ?>" method="POST" id="formValidarCodigo">
                    ...
                    <!-- Reenvio de código com contagem regressiva -->
...
<?= $this->section('scripts') ?>
    <!-- Script de Interação Assíncrona e OTP -->
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Configuração
            const URL_ENVIAR_CODIGO = '<?= base_url('receberemailvalidacao') ?>';
            const TEMPO_COUNTDOWN = 30; // 30 segundos solicitados
            ...
            async function enviarCodigoAssincrono(origemBtn) {
                ...
                try {
                    ...
                    // Se a resposta contiver novo CSRF token atualizado pelo CI4
                    ...
                    if (!response.ok || (data && data.success === false)) {
                        const erroBackend = (data && (data.message || data.error)) 
                            ...
                            : 'Ocorreu um erro ao tentar enviar o código para o seu e-mail.';
                    ...
                    const mensagemSucesso = (data && data.message) 
                        ? data.message 
                        : 'Código de verificação enviado com sucesso para seu e-mail!';
                    mostrarSucesso(mensagemSucesso);

                    // Oculta botão inicial e exibe os campos para informar o código recebido
                    sectionEnviarCodigo.classList.add('d-none');
                    sectionInformarCodigo.classList.remove('d-none');

                    // Dispara a contagem de 15 segundos
            ...
            otpInputs.forEach((input, index) => {
                ...
                input.addEventListener('input', (e) => {
                    ...
                    if (input.value.length > 0) {
                        ...
                        // Avança para o próximo campo se existir
                ...
                // Suporte a Colar (Paste) ex: colar "123456" de uma vez
            ...
            formValidarCodigo.addEventListener('submit', async (e) => {
                ...
                // Desabilita inputs enquanto valida
                ...
                try {
                    ...
                    const urlDestino = (data && data.redirect)
                        ? data.redirect
                        : '<?= base_url('usuario' . strtolower(session('usuario')->tipo . '/criarconta')) ?>';
                    setTimeout(() => {
                    ...
                    // Reabilita os inputs com feedback de erro
                    ...
                    // Efeito de shake visual nos campos
